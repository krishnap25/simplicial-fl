...
def word_to_indices(word):
    """Returns a list of character indices, normalized by length of NUM_LETTERS

    Args:
        word: string

    Return:
        indices: float list with length len(word)
    """
    indices = []
    for c in word:
        indices.append(max(0., ALL_LETTERS.find(c)) / NUM_LETTERS)  # added max to account for -1
    return indices
